...
Пробовали gpt-4o (2026-07-15) вместо mini — смешанный результат, не однозначное улучшение
(sentiment лучше на одном клиенте/хуже на другом, tags хуже на обоих) и не работает как
решение "в лоб": если клиент потребует закрытый контур с локальной моделью, доступа к
топовым моделям не будет вообще — промпт/архитектура должны быть надёжны и на слабой модели,
не полагаться на масштаб модели как компенсацию нечёткого промпта. Вернули mini, см. PLAN.md
для разбора первопричины (пробел в промпте/структуре словаря, не в модели).
"""
import os
